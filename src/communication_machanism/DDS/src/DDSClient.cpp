...
bool DDSClient::init(std::string service_name)
{
    ...
    mp_result_reader = mp_result_sub->create_datareader(
        mp_result_topic, rqos, &this->m_resultsListener);
...
Serialization *DDSClient::call_service(Serialization *param, int enclave_id)
{
    ...
    // do {
    const int MaxFragmentSize = 64 * 1024;
    const int TotalFragment =
        (m_operation.m_vector.size() + MaxFragmentSize - 1) / MaxFragmentSize;

    m_operation.total_fragment = TotalFragment;
    std::vector<char> bytes_to_send;
    bytes_to_send.swap(m_operation.m_vector);

    std::unordered_set<int> received;
    int remain_size = bytes_to_send.size();
    for (int i = 0; i < TotalFragment; i++) {
        m_operation.fragment_idx = i;
        int size = std::min(MaxFragmentSize, remain_size);
        remain_size -= size;
        m_operation.m_vector_size = size;
        m_operation.m_vector = std::vector<char>(
            bytes_to_send.begin() + i * MaxFragmentSize,
            bytes_to_send.begin() + i * MaxFragmentSize + size);
        mp_operation_writer->write((char *)&m_operation);
    ...
    //              eprosima::fastdds::dds::ALIVE_INSTANCE_STATE ||
    //          m_result.m_guid != m_operation.m_guid ||
    //          received.size() != TotalFragment);

    std::cout << "READ DONE: ENCLAVE_ID = " << m_result.m_enclave_id
              << std::endl;
