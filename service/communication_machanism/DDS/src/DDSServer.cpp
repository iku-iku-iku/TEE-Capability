...
void DDSServer::sendMessageToRibbon(eprosima::fastdds::dds::DataWriter *writer_detect,
                                    eprosima::fastdds::dds::DataReader *reader_detect)
{
    ...
    do {
        ...
        reader_detect->wait_for_unread_message(
            { RETRY_COUNT, 0 });
...
void DDSServer::OperationListener::on_data_available(DataReader * /* reader */)
{
    ...
    if (m_sampleInfo.valid_data) {
        ...
        if (operation_type != NOTIFICATION_MESSAGE) {
            ...
            Serialization *result = server->call_(funname, ds.current(),
                                                  ds.size() - funname.size());
...
bool DDSRouter::first_add_server(std::string server_guid, eprosima::fastdds::dds::DataReader *&result_reader,
                                 ...
                                 eprosima::fastdds::dds::Topic *&result_topic)
{
    ...
    operation_writer = mp_result_pub->create_datawriter(
        operation_topic, wqos, listener);
    ...
    result_topic = mp_participant->create_topic(
        mp_result_topic_server_name, "Result", TOPIC_QOS_DEFAULT);
...
bool DDSRouter::non_first_add_server(std::string server_guid, eprosima::fastdds::dds::DataReader *&result_reader,
                                     eprosima::fastdds::dds::DataWriter *&operation_writer,
                                     eprosima::fastdds::dds::Topic *&operation_topic,
                                     eprosima::fastdds::dds::Topic *&result_topic)
{
    ...
    OperationServerListener *listener = new OperationServerListener(this);
    m_operationsServerListenerList.push_back(listener);
    operation_writer = mp_operation_pub->create_datawriter(
        operation_topic, wqos, listener);
    if (operation_writer == nullptr) {
        return false;
    }
...
bool DDSRouter::add_server(std::string server_guid)
{
    ...
    if (call_times == ONE) {
        if (!first_add_server(server_guid, result_reader, operation_writer, operation_topic, result_topic)) {
            return false;
        }
    } else if (call_times >= TWO) {
        if (!non_first_add_server(server_guid, result_reader, operation_writer, operation_topic, result_topic)) {
            return false;
        }
    } else {
        return false;
